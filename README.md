...
- **Expressions** — expression content (e.g. `expression`, `imageExpression`) editable in a **multi-line textarea** for model-driven elements; a toggle button expands or collapses the textarea for longer expressions; commit and newline behavior is configurable (see [Expression Editing](#expression-editing)); an **Open in Editor** button opens the expression in a full VS Code editor tab (side-by-side) with syntax highlighting for the report's language (`java`, `groovy`, or `javascript`); saving the editor tab writes the expression back to the JRXML; editing an existing expression updates its CDATA content, clearing removes the child element, and typing into an absent expression creates a new child element with CDATA wrapping
...
- **Keyboard shortcuts** — attributes commit on **Enter** or blur; expression textarea behavior depends on the `jasperreports.expression.enterBehavior` setting (see below); **Escape** reverts in both cases

### Expression Editing

The `jasperreports.expression.enterBehavior` setting controls what the **Enter** key does inside expression textareas:

| Value              | Enter   | Shift+Enter | Ctrl/Cmd+Enter | Blur    |
| ------------------ | ------- | ----------- | -------------- | ------- |
| `commit` (default) | Commits | Newline     | Commits        | Commits |
| `newline`          | Newline | Newline     | Commits        | Commits |
